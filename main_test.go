...
func TestParseAttachArgs(t *testing.T) {
	...
	host, pattern, o = parseAttachArgs("attach", []string{"-r", "--vhub", "root@h", "3-2"})
	if host != "root@h" || pattern != "3-2" || !o.reverse || !o.vhub {
		t.Errorf("parseAttachArgs(-r) = %q %q %+v", host, pattern, o)
	}
}
